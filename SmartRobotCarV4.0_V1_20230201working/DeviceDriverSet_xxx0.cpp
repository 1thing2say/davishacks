...
void DeviceDriverSet_Key::DeviceDriverSet_Key_Init(void)
{
  ...
  attachInterrupt(digitalPinToInterrupt(PIN_Key), attachPinChangeInterrupt_GetKeyValue, FALLING);
}
...
/*IRrecv - Using modern IRremote v4.x library with IrReceiver singleton*/
void DeviceDriverSet_IRrecv::DeviceDriverSet_IRrecv_Init(void)
{
  IrReceiver.begin(RECV_PIN, ENABLE_LED_FEEDBACK); //Enable infrared communication
}
