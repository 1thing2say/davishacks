...
// On R4 WiFi: Serial = USB, Serial1 = pins 0/1 (Bluetooth module)
...
enum SmartRobotCarFunctionalModel {
  ...
  CMD_ClearAllFunctions_Standby_mode, /*Clear All Functions And Enter Standby
                                         Mode*/
  CMD_ClearAllFunctions_Programming_mode, /*Clear All Functions And Enter
                                             Programming Mode*/
  CMD_MotorControl,                       /*Motor Control Mode*/
  CMD_CarControl_TimeLimit,   /*Car Movement Direction Control With Time Limit*/
  CMD_CarControl_NoTimeLimit, /*Car Movement Direction Control Without Time
                                 Limit*/
...
// PWM trim to compensate for mechanical asymmetry (motor mismatch,
// wheel diameter differences, slightly bent chassis, etc.).
//   If the car drifts RIGHT when going straight: increase  (+5, +10, +15...)
//   If the car drifts LEFT  when going straight: decrease  (-5, -10, -15...)
// Tune by ±5 at a time. Values rarely need to exceed ±20.
#define MOTOR_TRIM 70
...
void ApplicationFunctionSet::ApplicationFunctionSet_Init(void) {
  bool res_error = true;
  Serial.begin(921600);  // USB serial — high baud for image transfer
  BT_Serial.begin(9600); // Bluetooth module on pins 0/1
...
/*
  Straight line movement control：For dual-drive motors, due to frequent motor
  coefficient deviations and many external interference factors, it is difficult
  for the car to achieve relative Straight line movement. For this reason, the
  feedback of the yaw control loop is added. direction：only forward/backward
  directionRecord：Used to update the direction and position data (Yaw value)
  when entering the function for the first time. speed：the speed range is 0~255
  Kp：Position error proportional constant（The feedback of improving location
  resuming status，will be modified according to different mode），improve
  damping control. UpperLimit：Maximum output upper limit control
*/
static void ApplicationFunctionSet_SmartRobotCarLinearMotionControl(
    SmartRobotCarMotionControl direction, uint8_t directionRecord,
    uint8_t speed, uint8_t Kp, uint8_t UpperLimit) {
  ...
  if (tilted) {
    ...
    R = (Yaw - yaw_So) * Kp + speed + MOTOR_TRIM;
    if (R > UpperLimit)
      R = UpperLimit;
    else if (R < 10)
      R = 10;
    L = (yaw_So - Yaw) * Kp + speed - MOTOR_TRIM;
    if (L > UpperLimit)
      L = UpperLimit;
    else if (L < 10)
      L = 10;
...
/*
  ...
  Input parameters:     1# direction:Forward（1）、Backward（2）、
  Left（3）、Right（4）、LeftForward（5）、LeftBackward（6）、RightForward（7）RightBackward（8）
                        2# speed(0--255)
*/
static void ApplicationFunctionSet_SmartRobotCarMotionControl(
    SmartRobotCarMotionControl direction, uint8_t is_speed) {
  ...
  // Control mode that requires straight line movement adjustment（Car will has
  // movement offset easily in the below mode，the movement cannot achieve the
  // effect of a relatively straight direction so it needs to add control
  // adjustment）
  ...
  case /* constant-expression */
    ...
    if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode) {
      AppMotor.DeviceDriverSet_Motor_control(
          ...
          /*controlED*/ control_enable); // Motor control
    } else { // When moving forward, enter the direction and position approach
  ...
  case /* constant-expression */ Backward:
    ...
    if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode) {
      AppMotor.DeviceDriverSet_Motor_control(
          ...
          /*controlED*/ control_enable); // Motor control
    } else { // When moving backward, enter the direction and position approach
             // control loop processing
      ApplicationFunctionSet_SmartRobotCarLinearMotionControl(
  ...
  case /* constant-expression */ RightForward:
    ...
    directionRecord = 7;
    AppMotor.DeviceDriverSet_Motor_control(
        /*direction_A*/ direction_just, /*speed_A*/ speed / 2,
        /*direction_B*/ direction_just, /*speed_B*/ speed,
        /*controlED*/ control_enable); // Motor control
    break;
  case /* constant-expression */ RightBackward:
    ...
    directionRecord = 8;
    AppMotor.DeviceDriverSet_Motor_control(
        /*direction_A*/ direction_back, /*speed_A*/ speed / 2,
        /*direction_B*/ direction_back, /*speed_B*/ speed,
        /*controlED*/ control_enable); // Motor control
    break;
...
void ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void) {
  ...
  { /*Battery voltage status update*/
    ...
    if (millis() - VoltageData_time > 10) // read and update the data per 10ms
    {
      ...
      if (VoltageData_V < VoltageDetection) {
        VoltageData_number++;
        if (VoltageData_number ==
            500) // Continuity to judge the latest voltage value multiple
        {
  ...
  // { /*value updation for the ultrasonic sensor：for the Obstacle Avoidance
  // mode*/
  ...
  { /*value updation for the IR sensors on the line tracking module：for the
       line tracking mode*/
...
void ApplicationFunctionSet::ApplicationFunctionSet_RGB(void) {
  ...
  if (function_xxx((temp), 0, 500) && VoltageDetectionStatus == true) {
    ...
    case /* constant-expression */ 0 ... 49:
      ...
      AppRBG_LED.DeviceDriverSet_RBGLED_xxx(0 /*Duration*/,
                                            2 /*Traversal_Number*/, CRGB::Red);
      break;
    case /* constant-expression */ 50 ... 99:
      /* code */
      AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
          0 /*Duration*/, 2 /*Traversal_Number*/, CRGB::Black);
      break;
  ...
  } else if (((function_xxx((temp), 500, 3000)) &&
              VoltageDetectionStatus == true) ||
             VoltageDetectionStatus == false) {
    switch (Application_SmartRobotCarxxx0
                .Functional_Mode) // Act on mode control sequence
    {
    case /* constant-expression */ Standby_mode:
      /* code */
      {
        if (VoltageDetectionStatus == true) {
          AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
              0 /*Duration*/, 2 /*Traversal_Number*/, CRGB::Red);
          delay(30);
          AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
              0 /*Duration*/, 2 /*Traversal_Number*/, CRGB::Black);
          delay(30);
    ...
    case /* constant-expression */ TraceBased_mode:
      ...
      {
        AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
            0 /*Duration*/, 2 /*Traversal_Number*/, CRGB::Green);
      }
      break;
    case /* constant-expression */ ObstacleAvoidance_mode:
      ...
      {
        AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
            0 /*Duration*/, 2 /*Traversal_Number*/, CRGB::Yellow);
      }
      break;
    case /* constant-expression */ Follow_mode:
      ...
      {
        AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
            0 /*Duration*/, 2 /*Traversal_Number*/, CRGB::Blue);
      }
      break;
    case /* constant-expression */ Rocker_mode:
      ...
      {
        AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
            0 /*Duration*/, 2 /*Traversal_Number*/, CRGB::Violet);
      }
      break;
    default:
      break;
...
void ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void) {
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode) {
    ...
    // int getAnaloguexxx_L =
    // AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_L(); int
    // getAnaloguexxx_M =
    // AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_M(); int
    // getAnaloguexxx_R =
    ...
    } else ////The car is not on the black line. execute Blind scan
    {
      ...
      if ((function_xxx((millis() - MotorRL_time), 0, 200) ||
        ...
        ApplicationFunctionSet_SmartRobotCarMotionControl(Right, 100);
                 ...
                 BlindDetection == true) {
        ApplicationFunctionSet_SmartRobotCarMotionControl(Left, 100);
      } else if ((function_xxx((millis() - MotorRL_time), 3000,
                               3500))) // Blind Detection ...s ?
      {
...
void ApplicationFunctionSet::ApplicationFunctionSet_Follow(void) {
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == Follow_mode) {
    ...
    if (false == function_xxx(ULTRASONIC_Get, 0,
                              20)) // There is no obstacle 20 cm ahead?
    {
      ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
      ...
      if (timestamp == 3) {
        if (Position_Servo_xx !=
            Position_Servo) // Act on servo motor：avoid loop execution
        {
          Position_Servo_xx =
              Position_Servo; // Act on servo motor：rotation angle record
      ...
      if (millis() - time_Servo >
          1000) // Act on servo motor：stop at the current location for 2s
      {
        timestamp = 3;
        Position_Servo += 1;
        OneCycle += 1;
...
void ApplicationFunctionSet::ApplicationFunctionSet_Servo(uint8_t Set_Servo) {
  ...
  case 1 ... 2: {
    ...
    AppServo.DeviceDriverSet_Servo_controls(
        /*uint8_t Servo--y*/ 2, /*unsigned int Position_angle*/ y_angle);
  } break;
  ...
  case 3 ... 4: {
    ...
    AppServo.DeviceDriverSet_Servo_controls(
        /*uint8_t Servo--z*/ 1, /*unsigned int Position_angle*/ z_angle);
  } break;
  case 5:
    AppServo.DeviceDriverSet_Servo_controls(/*uint8_t Servo--y*/ 2,
                                            /*unsigned int Position_angle*/ 9);
    AppServo.DeviceDriverSet_Servo_controls(/*uint8_t Servo--z*/ 1,
                                            /*unsigned int Position_angle*/ 9);
    break;
  default:
    break;
...
void ApplicationFunctionSet::ApplicationFunctionSet_Standby(void) {
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == Standby_mode) {
    ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
    if (true == is_ED) // Used to zero yaw raw data(Make sure the car is placed
...
/*
  N1:command
  CMD mode：Car receive the control commands from the APP<motor control
  command>,perform unidirectional drive of the motor Input parameters: uint8_t
  CMD_is_MotorSelection,  Motor selection  1left motor  2right motor  0both
  motors uint8_t CMD_is_MotorDirection,  rotation direction selection  1forward
  2backward  0stop uint8_t CMD_is_MotorSpeed,      motor speed  0-250 No time
  limited
*/
void ApplicationFunctionSet::CMD_MotorControl_xxx0(uint8_t is_MotorSelection,
                                                   uint8_t is_MotorDirection,
...
/*
  N2：command
  CMD mode：Receive the control commands from the APP,perform movement direction
  and speed control of the car Time limited
*/
void ApplicationFunctionSet::CMD_CarControlTimeLimit_xxx0(
    uint8_t is_CarDirection, uint8_t is_CarSpeed, uint32_t is_Timer) {
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode ==
      CMD_CarControl_TimeLimit) // enter time-limited control mode
  {
    CarControl = true;
    if (is_Timer != 0) // #1 if the pre-set time is not ... (zero)
    {
      if ((millis() - Application_SmartRobotCarxxx0.CMD_CarControl_Millis) >
          (is_Timer)) // check the timestamp
      {
        CarControl_TE = true;
        ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);

        Application_SmartRobotCarxxx0.Functional_Mode =
            CMD_Programming_mode; /*set mode to programming mode<Waiting for the
                                     next set of control commands>*/
        if (CarControl_return == false) {

#if _is_print
          BT_Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
...
void ApplicationFunctionSet::CMD_CarControlTimeLimit_xxx0(void) {
      ...
      CMD_CarControl_TimeLimit) // enter time-limited control mode
  {
    ...
    {
      if ((millis() - Application_SmartRobotCarxxx0.CMD_CarControl_Millis) >
          (CMD_is_CarTimer)) // check the timestamp
      {
        CarControl_TE = true;
        ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
...
/*
  N3 command
  CMD mode：Receive the control commands from the APP,perform movement direction
  and speed control of the car No time limited
*/
void ApplicationFunctionSet::CMD_CarControlNoTimeLimit_xxx0(
    uint8_t is_CarDirection, uint8_t is_CarSpeed) {
  static boolean CarControl = false;
  if (Application_SmartRobotCarxxx0.Functional_Mode ==
      CMD_CarControl_NoTimeLimit) // enter no time-limited control mode
  {
    CarControl = true;
    CMD_CarControl(is_CarDirection, is_CarSpeed);
  } else {
...
/*
  ...
  Receive the control commands from the APP,perform motion control of the left
  and right motors
*/
void ApplicationFunctionSet::CMD_MotorControlSpeed_xxx0(uint8_t is_Speed_L,
                                                        uint8_t is_Speed_R) {
  static boolean MotorControl = false;
  if (Application_SmartRobotCarxxx0.Functional_Mode == CMD_MotorControl_Speed) {
    ...
    if (is_Speed_L == 0 && is_Speed_R == 0) {
      ...
      AppMotor.DeviceDriverSet_Motor_control(
          /*direction_A*/ direction_just, /*speed_A*/ is_Speed_L,
...
void ApplicationFunctionSet::CMD_MotorControlSpeed_xxx0(void) {
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == CMD_MotorControl_Speed) {
    ...
    if (CMD_is_MotorSpeed_L == 0 && CMD_is_MotorSpeed_R == 0) {
      ...
      AppMotor.DeviceDriverSet_Motor_control(
          /*direction_A*/ direction_just, /*speed_A*/ CMD_is_MotorSpeed_L,
...
void ApplicationFunctionSet::CMD_ServoControl_xxx0(void) {
  if (Application_SmartRobotCarxxx0.Functional_Mode == CMD_ServoControl) {
    AppServo.DeviceDriverSet_Servo_controls(
        /*uint8_t Servo*/ CMD_is_Servo,
        /*unsigned int Position_angle*/ CMD_is_Servo_angle / 10);
    Application_SmartRobotCarxxx0.Functional_Mode =
        CMD_Programming_mode; /*set mode to programming mode<Waiting for the
                                 next set of control commands>*/
  }
...
*/
void ApplicationFunctionSet::CMD_LightingControlTimeLimit_xxx0(
    uint8_t is_LightingSequence, uint8_t is_LightingColorValue_R,
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode ==
      CMD_LightingControl_TimeLimit) // enter time-limited control mode
  {
    LightingControl = true;
    if (is_LightingTimer != 0) // #1 if the pre-set time is not ... (zero)
    {
      if ((millis() -
           Application_SmartRobotCarxxx0.CMD_LightingControl_Millis) >
          (is_LightingTimer)) // Check the timestamp
      {
    ...
    if (LightingControl_TE == false) {
      CMD_Lighting(is_LightingSequence, is_LightingColorValue_R,
                   is_LightingColorValue_G, is_LightingColorValue_B);
    }
  } else {
...
void ApplicationFunctionSet::CMD_LightingControlTimeLimit_xxx0(void) {
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode ==
      CMD_LightingControl_TimeLimit) // Enter Lighting Control mode with
                                     // time-limited
  {
    LightingControl = true;
    if (CMD_is_LightingTimer != 0) // #1 if the pre-set time is not ... (zero)
    {
      if ((millis() -
           Application_SmartRobotCarxxx0.CMD_LightingControl_Millis) >
          (CMD_is_LightingTimer)) // Check the timestamp
      {
        LightingControl_TE = true;
        FastLED.clear(true);
        Application_SmartRobotCarxxx0.Functional_Mode =
...
/*
  ...
  No time limited
*/
void ApplicationFunctionSet::CMD_LightingControlNoTimeLimit_xxx0(
    uint8_t is_LightingSequence, uint8_t is_LightingColorValue_R,
    uint8_t is_LightingColorValue_G, uint8_t is_LightingColorValue_B) {
  static boolean LightingControl = false;
  if (Application_SmartRobotCarxxx0.Functional_Mode ==
      CMD_LightingControl_NoTimeLimit) // Enter Lighting Control mode without
                                       // time-limited
  {
    LightingControl = true;
    CMD_Lighting(is_LightingSequence, is_LightingColorValue_R,
...
void ApplicationFunctionSet::CMD_ClearAllFunctions_xxx0(void) {
  if (Application_SmartRobotCarxxx0.Functional_Mode ==
      CMD_ClearAllFunctions_Standby_mode) // Command:N100 Clear all functions to
                                          // enter standby mode
  {
    ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
    FastLED.clear(true);
    AppRBG_LED.DeviceDriverSet_RBGLED_xxx(
        0 /*Duration*/, NUM_LEDS /*Traversal_Number*/, CRGB::Black);
    Application_SmartRobotCarxxx0.Motion_Control = stop_it;
    Application_SmartRobotCarxxx0.Functional_Mode = Standby_mode;
  }
  if (Application_SmartRobotCarxxx0.Functional_Mode ==
      CMD_ClearAllFunctions_Programming_mode) // Command:N110 Clear all
                                              // functions and enter programming
                                              // mode
  {
...
/*
  N21:command
  CMD mode：The ultrasonic module receives and feeds back the status and ranging
  data according to the control command of APP terminal. Input：
*/
void ApplicationFunctionSet::CMD_UltrasoundModuleStatus_xxx0(uint8_t is_get) {
  AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(
      (uint16_t *)&UltrasoundData_cm /*out*/); // Ultrasonic data
  UltrasoundDetectionStatus =
      function_xxx(UltrasoundData_cm, 0, ObstacleDetection);
  if (1 == is_get) // ultrasonic sensor  is_get Start     true：has obstacle /
                   // false: no obstable
  {
    if (true == UltrasoundDetectionStatus) {
#if _is_print
      BT_Serial.print('{' + CommandSerialNumber + "_true}");
#endif
    } else {
#if _is_print
      BT_Serial.print('{' + CommandSerialNumber + "_false}");
#endif
    }
...
#if _is_print
    BT_Serial.print('{' + CommandSerialNumber + '_' + toString + '}');
#endif
  }
}
/*
  N22:command
  CMD mode：Tracking module receives and feeds back tracing status and data
  according to the control command of APP terminal Input：
*/
void ApplicationFunctionSet::CMD_TraceModuleStatus_xxx0(uint8_t is_get) {
  ...
  Application_SmartRobotCarxxx0.Functional_Mode =
      CMD_Programming_mode; /*set mode to programming mode<Waiting for the next
...
void ApplicationFunctionSet::ApplicationFunctionSet_IRrecv(void) {
  ...
  if (true == IRrecv_en) {
    ...
    case /* constant-expression */ 6:
      /* code */ Application_SmartRobotCarxxx0.Functional_Mode =
          TraceBased_mode;
      break;
    ...
    case /* constant-expression */ 9:
      /* code */ if (Application_SmartRobotCarxxx0.Functional_Mode ==
                     TraceBased_mode) // Adjust the threshold of the line
                                      // tracking module to adapt the actual
                                      // environment
      {
...
void ApplicationFunctionSet::ApplicationFunctionSet_SerialPortDataAnalysis(
  ...
  if (BT_Serial.available() > 0) {
    while (c != '}' && BT_Serial.available() > 0) {
      // while (BT_Serial.available() == 0)//Forcibly wait for a frame of data
      // to be received
      //   ;
      c = BT_Serial.read();
  ...
  if (c == '}') // Data frame tail check
  {
...
#endif
    // if (true == SerialPortData.equals("{f}") || true ==
    // SerialPortData.equals("{b}") || true == SerialPortData.equals("{l}") ||
    // true == SerialPortData.equals("{r}"))
    // {
    //   Serial.print(SerialPortData);
    //   SerialPortData = "";
    //   return;
    // }
    // if (true == SerialPortData.equals("{Factory}") || true ==
    // SerialPortData.equals("{WA_NO}") || true ==
    ...
    DeserializationError error = deserializeJson(
        doc,
        SerialPortData); // Deserialize JSON data from the serial data buffer
    SerialPortData = "";
    if (error) {
    ...
    } else if (!error) // Check if the deserialization is successful
    {
      ...
      /*Please view the following code blocks in conjunction with the
      ...
      case 2: /*<Command：N 2> */
        Application_SmartRobotCarxxx0.Functional_Mode =
            CMD_CarControl_TimeLimit; /*Car movement direction and speed
                                         control：Time limited mode*/
      ...
      case 3: /*<Command：N 3> */
        Application_SmartRobotCarxxx0.Functional_Mode =
            CMD_CarControl_NoTimeLimit; /*Car movement direction and speed
                                           control：No time limited mode*/
        CMD_is_CarDirection = doc["D1"];
        CMD_is_CarSpeed = doc["D2"];
#if _is_print
        BT_Serial.print('{' + CommandSerialNumber + "_ok}");
      ...
      case 4: /*<Command：N 4> */
        Application_SmartRobotCarxxx0.Functional_Mode =
            CMD_MotorControl_Speed; /*motor control:Control motor speed mode*/
      ...
      case 7: /*<Command：N 7> */
        Application_SmartRobotCarxxx0.Functional_Mode =
            CMD_LightingControl_TimeLimit; /*Lighting control:Time limited
                                              mode*/
      ...
      case 8: /*<Command：N 8> */
        Application_SmartRobotCarxxx0.Functional_Mode =
            CMD_LightingControl_NoTimeLimit; /*Lighting control:No time limited
                                                mode*/
      ...
      case 110: /*<Command：N 110> */
        Application_SmartRobotCarxxx0.Functional_Mode =
            CMD_ClearAllFunctions_Programming_mode; /*Clear all function:Enter
                                                       programming mode*/
#if _is_print
        BT_Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
      ...
      case 100: /*<Command：N 100> */
        Application_SmartRobotCarxxx0.Functional_Mode =
            CMD_ClearAllFunctions_Standby_mode; /*Clear all function:Enter
                                                   standby mode*/
#if _is_print
        BT_Serial.print("{ok}");
        // Serial.print('{' + CommandSerialNumber + "_ok}");
      ...
      case 105: /*<Command：N 105> :FastLED brightness adjustment control
                   command*/
...
/* WiFi rocker-style drive command — mirrors N=102 from the JSON parser.
   rocker_dir: 1=Fwd 2=Back 3=Left 4=Right 5=FwdLeft 6=BackLeft 7=FwdRight
   8=BackRight 9=Stop */
void ApplicationFunctionSet::ApplicationFunctionSet_WiFiCommand(
    uint8_t rocker_dir, uint8_t speed) {
  Application_SmartRobotCarxxx0.Functional_Mode = Rocker_mode;
  Rocker_CarSpeed = speed;
  switch (rocker_dir) {
  case 1:
